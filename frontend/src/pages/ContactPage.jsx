...
const initialForm = { type: 'SUGGESTION', name: '', email: '', subject: '', message: '', rating: '' };

const ContactPage = () => {
  const [form, setForm] = useState(initialForm);
  const [status, setStatus] = useState({ loading: false, error: '', success: '' });

  const update = (event) => setForm((current) => ({ ...current, [event.target.name]: event.target.value, ...(event.target.name === 'type' && event.target.value !== 'REVIEW' ? { rating: '' } : {}) }));
      ...
      <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
        ...
        <form onSubmit={submit} className="border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
          <div className="mb-7 flex gap-2 border-b border-gray-200 pb-5">
            {['SUGGESTION', 'COMPLAINT', 'REVIEW'].map((type) => <label key={type} className={`flex-1 cursor-pointer border px-4 py-3 text-center text-sm font-semibold transition ${form.type === type ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-500 hover:border-gray-400'}`}><input type="radio" name="type" value={type} checked={form.type === type} onChange={update} className="sr-only" />{type === 'SUGGESTION' ? 'Suggestion' : type === 'COMPLAINT' ? 'Complaint' : 'Review'}</label>)}
          </div>
          ...
          {form.type === 'REVIEW' && <fieldset className="mt-5"><legend className="mb-2 block text-sm font-medium text-gray-700">Your rating</legend><div className="flex gap-1">{[1, 2, 3, 4, 5].map((rating) => <label key={rating} className="cursor-pointer"><input type="radio" name="rating" value={rating} checked={String(form.rating) === String(rating)} onChange={update} required className="sr-only" /><Star className={`h-7 w-7 ${Number(form.rating) >= rating ? 'fill-amber-400 text-amber-400' : 'text-gray-300'}`} /></label>)}</div></fieldset>}
          <button type="submit" disabled={status.loading} className="mt-6 inline-flex w-full items-center justify-center gap-2 bg-blue-600 px-5 py-3 font-semibold text-white transition hover:bg-blue-500 disabled:bg-gray-400">{status.loading ? 'Sending...' : 'Send message'} <Send className="h-4 w-4" /></button>
        </form>
      </div>
    </div>
  );
