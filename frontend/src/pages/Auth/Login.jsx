...
import { ArrowRight, CheckCircle2, LockKeyhole, MapPin, Phone, ShieldCheck, UserRound, Wrench } from 'lucide-react';
...
const Login = () => {
  ...
  return <div className="flex min-h-[calc(100vh-72px)] items-center justify-center bg-gray-50 px-6 py-12 sm:px-10"><motion.div className="grid w-full max-w-5xl overflow-hidden border border-gray-200 bg-white shadow-xl lg:grid-cols-[0.85fr_1.15fr]" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
    <aside className="hidden bg-[#111827] p-10 text-white lg:flex lg:flex-col lg:justify-between"><div><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center bg-blue-500"><Wrench className="h-5 w-5" /></div><p className="text-sm font-semibold uppercase tracking-[0.22em] text-blue-300">ToolKit</p></div><h1 className="mt-8 text-4xl font-bold leading-tight">Your personal toolkit, ready when you are.</h1><p className="mt-5 leading-7 text-gray-300">Keep your tools, delivery details, and orders connected in one place.</p><div className="mt-10 space-y-4 text-sm text-gray-300"><p className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-blue-300" />Fast checkout with saved details</p><p className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-blue-300" />Order tracking from dispatch to delivery</p><p className="flex items-center gap-3"><ShieldCheck className="h-5 w-5 text-blue-300" />A safer way to source worksite gear</p></div></div><div className="border-t border-gray-700 pt-5 text-sm text-gray-400"><p>Professional tools. Clear pricing. Simpler work.</p></div></aside>
    <main className="p-7 sm:p-10"><div className="mb-8 flex border-b border-gray-200"><button type="button" onClick={() => { setIsLogin(true); setError(''); }} className={`border-b-2 px-1 pb-3 text-sm font-semibold ${isLogin ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-400'}`}>Sign in</button><button type="button" onClick={() => { setIsLogin(false); setError(''); }} className={`ml-6 border-b-2 px-1 pb-3 text-sm font-semibold ${!isLogin ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-400'}`}>Create account</button></div><div><p className="text-sm font-semibold uppercase tracking-[0.16em] text-blue-600">{isLogin ? 'Welcome back' : 'New to ToolKit'}</p><h2 className="mt-2 text-3xl font-bold text-gray-900">{isLogin ? 'Sign in to continue' : 'Create your account'}</h2><p className="mt-2 text-gray-500">{isLogin ? 'Access your cart, orders, and saved details.' : 'Tell us where to reach you so checkout is faster.'}</p></div>
      <form className="mt-8 space-y-5" onSubmit={handleSubmit}>{error && <div className="border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</div>}
        ...
        <button type="submit" disabled={loading} className="flex w-full items-center justify-center gap-2 bg-blue-600 px-4 py-3 font-semibold text-white transition hover:bg-blue-500 disabled:bg-gray-400">{loading ? 'Please wait...' : isLogin ? 'Sign in' : 'Create account'} <ArrowRight className="h-4 w-4" /></button>
      </form><div className="my-7 flex items-center gap-3 text-xs uppercase tracking-widest text-gray-400"><span className="h-px flex-1 bg-gray-200" />or<span className="h-px flex-1 bg-gray-200" /></div><GoogleButton label={isLogin ? 'Continue with Google' : 'Create account with Google'} onSuccess={handleGoogle} onError={setError} />{isLogin && <div className="mt-5 text-center text-sm"><Link to="/forgot-password" className="font-semibold text-gray-500 hover:text-blue-600">Forgot password?</Link></div>}<p className="mt-8 text-center text-sm text-gray-500">{isLogin ? 'New here?' : 'Already have an account?'} <button type="button" onClick={() => { setIsLogin(!isLogin); setError(''); }} className="font-semibold text-blue-600 hover:underline">{isLogin ? 'Create an account' : 'Sign in instead'}</button></p>
    </main>
  </motion.div></div>;
};
