...
import { api, clearAuth, getAuth, money, setAuth } from '../../api';
...
const Dashboard = () => {
  ...
  const deleteProfile = async () => {
    ...
    await api('/auth/profile', { method: 'DELETE' }); clearAuth(); window.location.href = '/';
  };
    ...
    <div className="mx-auto max-w-6xl p-6">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        ...
        <form onSubmit={track} className="rounded border bg-white p-5 md:col-span-2">
          <label htmlFor="track-order-id" className="text-sm text-gray-600">Track by Order ID</label>
          <div className="mt-2 flex gap-2"><input id="track-order-id" value={trackId} onChange={(e) => setTrackId(e.target.value)} className="flex-1 rounded border px-3 py-2" placeholder="9-character ID" /><button type="submit" className="rounded bg-black px-4 text-white">Track</button></div>
      ...
      <form onSubmit={saveProfile} className="mt-8 rounded border bg-white p-5">
        ...
        <div className="mt-4 flex gap-3"><button type="submit" className="rounded bg-black px-4 py-2 text-sm font-semibold text-white">Save profile</button><button type="button" onClick={deleteProfile} className="rounded border border-red-200 px-4 py-2 text-sm text-red-700">Deactivate account</button></div>
      </form>
          ...
          <div key={order.id} className="rounded border bg-white p-4">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex gap-2">
                {['PENDING', 'CONFIRMED'].includes(order.status) && <button type="button" onClick={() => updateStatus(order.id, 'CANCELLED')} className="rounded border px-3 py-2 text-sm">Cancel</button>}
                {order.status === 'DELIVERED' && <button type="button" onClick={() => updateStatus(order.id, 'RETURN_REQUESTED')} className="rounded border px-3 py-2 text-sm">Request Return</button>}
              </div>
            </div>
