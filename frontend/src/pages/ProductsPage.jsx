...
const ProductsPage = () => {
  ...
  const [loaderReady, setLoaderReady] = useState(null);
  const queryKey = `${categoryQuery || ''}|${subcategory}|${searchQuery}`;
  const loading = loadedQuery !== queryKey;
  const showLoader = loaderReady === queryKey;

  useEffect(() => {
    const timer = window.setTimeout(() => setLoaderReady(queryKey), 1000);
  ...
  useEffect(() => {
    ...
    api(`/products?${params}`).then((data) => {
      if (active) { resolved = true; setProducts(data.length ? data : localProducts); setInventoryReady(true); setMessage(''); }
    }).catch((err) => {
      if (active) {
        resolved = true;
        setProducts(localProducts);
  ...
  const add = async (productId) => {
    ...
    try {
      void api('/cart/items', { method: 'POST', body: JSON.stringify({ productId, quantity: 1, sale: saleView }) }).catch((err) => {
        setMessage(err.message);
        window.dispatchEvent(new CustomEvent('cart-change', { detail: { delta: -1 } }));
      });
        ...
        {products.map((product) => (
          ...
          >
            ...
            <div className="p-4">
              ...
              <button type="button" disabled={!inventoryReady || product.stock < 1 || adding === product.id} onClick={() => add(product.id)} className="w-full mt-4 bg-black text-white py-2 rounded hover:bg-gray-800 disabled:bg-gray-400 transition">
